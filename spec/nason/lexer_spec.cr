...
private def it_lexes(string, expected_kind : NASON::Token::Kind, expected_to_s = string, file = __FILE__, line = __LINE__)
  it "lexes #{string} from string", file, line do
    lexer = NASON::Lexer.new string
  ...
  it "lexes #{string} from IO", file, line do
    lexer = NASON::Lexer.new IO::Memory.new(string)
    token = lexer.next_token
    token.kind.should eq(expected_kind)
    token.to_s.should eq(expected_to_s)
  end
...
private def it_lexes_string(string, string_value, file = __FILE__, line = __LINE__)
  it "lexes #{string} from String", file, line do
    lexer = NASON::Lexer.new string
    token = lexer.next_token
    token.kind.should eq(NASON::Token::Kind::String)
...
private def it_lexes_int(string, int_value, file = __FILE__, line = __LINE__)
  it "lexes #{string} from String", file, line do
    lexer = NASON::Lexer.new string
    token = lexer.next_token
    token.kind.should eq(NASON::Token::Kind::Int)
...
private def it_lexes_float(string, float_value, file = __FILE__, line = __LINE__)
  it "lexes #{string} from String", file, line do
    lexer = NASON::Lexer.new string
    token = lexer.next_token
    token.kind.should eq(NASON::Token::Kind::Float)
...
private def it_errors_to_lex(string, *, file = __FILE__, line = __LINE__)
  it "errors if lexing #{string} from String", file: file, line: line do
    expect_raises(Exception) { NASON::Lexer.new(string).next_token }
  end

  it "errors if lexing #{string} from IO", file: file, line: line do
    expect_raises(Exception) { NASON::Lexer.new(IO::Memory.new(string)).next_token }
  end
end

describe NASON::Lexer do
  ...
  it_lexes_float "9.91343313498688", 9.913_433_134_986_88
  it_lexes_int "-1", -1
  it_lexes_float "-1.23", -1.23
  it_lexes_float "-1.23e4", -1.23e4
  it_lexes_float "-1.23e4", -1.23e4
  it_lexes_float "1000000000000000000.0", 1_000_000_000_000_000_000.0
  it_lexes_float "6000000000000000000.0", 6_000_000_000_000_000_000.0
  it_lexes_float "9000000000000000000.0", 9_000_000_000_000_000_000.0
  it_lexes_float "9876543212345678987654321.0", 9_876_543_212_345_678_987_654_321.0
  it_lexes_float "9876543212345678987654321e20", 9_876_543_212_345_678_987_654_321e20
