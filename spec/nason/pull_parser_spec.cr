...
class NASON::PullParser
  ...
  def assert(value : Float)
    kind.should eq(NASON::PullParser::Kind::Float)
    float_value.should eq(value)
    read_next
  end

  def assert(value : Bool)
    kind.should eq(NASON::PullParser::Kind::Bool)
    bool_value.should eq(value)
    read_next
  end

  def assert(value : String)
    kind.should eq(NASON::PullParser::Kind::String)
  ...
  def assert_array
    kind.should eq(NASON::PullParser::Kind::BeginArray)
    read_next
    yield
    kind.should eq(NASON::PullParser::Kind::EndArray)
    read_next
  end
  ...
  def assert_object
    kind.should eq(NASON::PullParser::Kind::BeginObject)
    read_next
    yield
    kind.should eq(NASON::PullParser::Kind::EndObject)
    read_next
  end
  ...
  def assert_error
    expect_raises NASON::ParseException do
      read_next
    end
  end
end
...
private def assert_pull_parse(string)
  it "parses #{string}" do
    parser = NASON::PullParser.new string
    parser.assert NASON.parse(string).raw
    parser.kind.should eq(NASON::PullParser::Kind::EOF)
  end
end
...
private def assert_raw(string, file = __FILE__, line = __LINE__)
  it "parses raw #{string.inspect}", file, line do
    pull = NASON::PullParser.new(string)
...
describe NASON::PullParser do
  ...
  it "parses when the input IO is already empty" do
    NASON::PullParser.new(IO::Memory.new).kind.should eq NASON::PullParser::Kind::EOF
  end

  it "prevents stack overflow for arrays" do
    parser = NASON::PullParser.new(("[" * 513) + ("]" * 513))
    expect_raises NASON::ParseException, "Nesting of 513 is too deep" do
      while true
  ...
  it "prevents stack overflow for hashes" do
    parser = NASON::PullParser.new((%({"x": ) * 513) + ("}" * 513))
    expect_raises NASON::ParseException, "Nesting of 513 is too deep" do
      while true
        break if parser.kind.eof?
        parser.read_next
  ...
  describe "skip" do
    [
      ...
      it "skips #{desc}" do
        pull = NASON::PullParser.new("[1, #{obj}, 2]")
        pull.read_array do
  ...
  it "reads array or null" do
    NASON::PullParser.new("null").read_array_or_null { fail "expected block not to be called" }

    pull = NASON::PullParser.new(%([1]))
  ...
  it "reads object or null" do
    NASON::PullParser.new("null").read_object_or_null { fail "expected block not to be called" }

    pull = NASON::PullParser.new(%({"foo": 1}))
  ...
  describe "on key" do
    it "finds key" do
      pull = NASON::PullParser.new(%({"foo": 1, "bar": 2}))
    ...
    ["1", "[1]", %({"x": [1]})].each do |value|
      it "yields all keys when skipping #{value}" do
        pull = NASON::PullParser.new(%({"foo": #{value}, "bar": 2}))
  ...
  describe "read?" do
    {% for pair in [[Int8, 1_i8],
      ...
      it "returns nil instead of {{type}} when the token is not compatible" do
        pull = NASON::PullParser.new(%({"foo": "bar"}))
    ...
    it "reads > Int64::MAX" do
      pull = NASON::PullParser.new(Int64::MAX.to_s + "0")
      pull.read?(Int64).should be_nil
    end

    it "reads < Int64::MIN" do
      pull = NASON::PullParser.new(Int64::MIN.to_s + "0")
      pull.read?(Int64).should be_nil
    end

    it "reads > Float32::MAX" do
      pull = NASON::PullParser.new(Float64::MAX.to_s)
      pull.read?(Float32).should be_nil
    end

    it "reads < Float32::MIN" do
      pull = NASON::PullParser.new(Float64::MIN.to_s)
      pull.read?(Float32).should be_nil
    end

    it "reads > UInt64::MAX" do
      pull = NASON::PullParser.new(UInt64::MAX.to_s + "0")
    ...
    it "reads > Float64::MAX" do
      pull = NASON::PullParser.new("1" + Float64::MAX.to_s)
    ...
    it "doesn't accept nan or infinity" do
      pull = NASON::PullParser.new(%("nan"))
      pull.read?(Float64).should be_nil

      pull = NASON::PullParser.new(%("infinity"))
      pull.read?(Float64).should be_nil
  ...
  it "#raise" do
    pull = NASON::PullParser.new("[1, 2, 3]")
    expect_raises(NASON::ParseException, "foo bar at line 1, column 2") do
      pull.raise "foo bar"
    end
    pull.read_begin_array
    expect_raises(NASON::ParseException, "foo bar at line 1, column 3") do
      pull.raise "foo bar"
    end
  end
end
