...
# an instance of `self`. This simply creates a `parser = NASON::PullParser`
# and invokes `new(parser)`: classes that want to provide NASON
...
# Deserializes the given NASON in *string_or_io* into
# an instance of `self`, assuming the NASON consists
...
# Parses a `String` or `IO` denoting a NASON array, yielding
...
def Float32.new(pull : NASON::PullParser)
  case pull.kind
  when .int?
...
def Float64.new(pull : NASON::PullParser)
  case pull.kind
  when .int?
...
def String.new(pull : NASON::PullParser)
  pull.read_string
end

def Path.new(pull : NASON::PullParser)
...
def Array.new(pull : NASON::PullParser)
  ary = new
  new(pull) do |element|
    ary << element
  end
...
def Deque.new(pull : NASON::PullParser)
  ary = new
  new(pull) do |element|
    ary << element
  end
...
def Set.new(pull : NASON::PullParser)
  set = new
  pull.read_array do
    set << T.new(pull)
  end
...
# If `nil` is returned a `NASON::ParseException` is raised.
#
# Values are parsed using the regular `new(pull : NASON::PullParser)` method.
def Hash.new(pull : NASON::PullParser)
  hash = new
  pull.read_object do |key, key_location|
    parsed_key = K.from_json_object_key?(key)
    unless parsed_key
      raise NASON::ParseException.new("Can't convert #{key.inspect} into #{K}", *key_location)
    end
...
def Tuple.new(pull : NASON::PullParser)
  {% begin %}
...
def NamedTuple.new(pull : NASON::PullParser)
  {% begin %}
    {% for key in T.keys %}
      %var{key.id} = nil
      %found{key.id} = false
    ...
    {% for key, type in T %}
      if %var{key.id}.nil? && !%found{key.id} && !{{type.nilable?}}
        raise NASON::ParseException.new("Missing json attribute: #{{{key.id.stringify}}}", *location)
      end
    {% end %}
...
# converter with `NASON::Field` and `YAML::Field`
#
# ```
# require "nason"
# require "yaml"
...
def Union.new(pull : NASON::PullParser)
  location = pull.location
  ...
  {% begin %}
    ...
    {% else %}
      ...
      {% end %}
      raise NASON::ParseException.new("Couldn't parse #{self} from #{string}", *location)
    {% end %}
  {% end %}
end

# Reads a string from NASON parser as a time formatted according to [RFC 3339](https://tools.ietf.org/html/rfc3339)
