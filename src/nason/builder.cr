...
#
# A `NASON::Error` is raised if attempting to generate an invalid NASON
...
class NASON::Builder
  ...
  # than this will result in a NASON::Error. Changing the value of this property
  ...
  def start_document : Nil
    ...
    when DocumentEndState
      ...
      raise NASON::Error.new("Starting document before ending previous one")
    end
  end
  ...
  def end_document : Nil
    ...
    when DocumentStartState
      raise NASON::Error.new("Empty NASON")
    when ArrayState
      raise NASON::Error.new("Unterminated NASON array")
    when ObjectState
      raise NASON::Error.new("Unterminated NASON object")
    when DocumentEndState
      # okay
    end
    flush
  ...
  def number(number : Float) : Nil
    scalar do
      case number
      when .nan?
        raise NASON::Error.new("NaN not allowed in NASON")
      when .infinite?
        raise NASON::Error.new("Infinity not allowed in NASON")
      else
        @io << number
      end
    end
  ...
  # might lead to invalid NASON being generated, so you must
  # be sure that *string* contains a valid NASON string.
  ...
  def end_array : Nil
    ...
    when ArrayState
      ...
      raise NASON::Error.new("Can't do end_array: not inside an array")
    end
    write_indent state
    @io << ']'
    decrease_indent
  ...
  def end_object : Nil
    ...
    when ObjectState
      unless state.name
        raise NASON::Error.new("Missing object value")
      end
      @state.pop
    else
      raise NASON::Error.new("Can't do end_object: not inside an object")
    end
    write_indent state
    @io << '}'
    decrease_indent
  ...
  private def start_scalar(string = false)
    ...
    when DocumentStartState
      # okay
    when StartState
      raise NASON::Error.new("Write before start_document")
    when DocumentEndState
      raise NASON::Error.new("Write past end_document and before start_document")
    when ArrayState
      comma unless state.empty
    when ObjectState
      if state.name && !string
        raise NASON::Error.new("Expected string for object name")
      end
  ...
  private def increase_indent
    ...
    if @current_indent > @max_nesting
      raise NASON::Error.new("Nesting of #{@current_indent} is too deep")
    end
  end
